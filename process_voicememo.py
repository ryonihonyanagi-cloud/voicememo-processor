...
- 【引用型】: 会話の中の印象的な言葉を軸に
- 【洞察型】: 抽象度を上げた深い考察
- 【Xスレッド型】: Hook→本論→CTAの構成で完結する数ツイート分（JSON内では1つの文字列として改行等で区切って表現）
- 【Threads用ロング】: 500文字程度で思考プロセスごと書く長文ポスト

プラットフォームはX（単発は140文字以内）、Xスレッド、Threads（500文字以内）のいずれかで指定してください。

【SNSライティング・ルールの厳守】
1. 完全な人間らしさ（AIっぽさゼロ）: ユーザーのプロフィールと過去の投稿例から、トーン・語尾・改行のクセ・ユーモアの入れ方を完全に模倣してください。
2. X向けの最適化: 最初の1文に強力なHook（問いかけ・共感・意外性）を入れ、絵文字は全体で1〜3個以内に抑えてください。
3. スパム表現の禁止: 「今すぐ」「激アツ」「人生変わる」「〜してみた結果」などの過剰な煽り文句は絶対に避けてください。
4. 自然な日本語: 翻訳調を避け、親しみやすさと少しの毒っ気やウィットを意識してください。"""
...
{{
  ...
  "x_threads_posts": [
    {{
      "platform": "X または Xスレッド または Threads",
      "type": "気づき型・問いかけ型・意見型・ストーリー型・引用型・洞察型・Xスレッド型・Threads用ロング のいずれか",
      "content": "ポスト文（X=140文字以内 / Threads=500文字以内 / Xスレッド=適宜改行）"
    }}
  ]
}}
